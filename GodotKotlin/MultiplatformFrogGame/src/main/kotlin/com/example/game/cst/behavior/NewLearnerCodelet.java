...
public class LearnerCodelet extends Codelet {
    ...
    LearnerCodelet(MemoryObject stateMO, FrogMindCommunicator communicator,
                   ...
                   ValueBasedRL learning, Domain[3]actionSpace,
    ...
    public void proc() {
        if (this.currEpisode < this.numEpisodes) {
            if (this.currStep == 0 && this.currEpisode == 0) {
                if (Files.exists(
                        this.pathToSaveLearning + this.learningFileName)) {
                    if (this.isTabular)
            ...
            if(!this.episodeIsDone) {
                ...
                if (isTabular)
                    idAction =
                            qLearning.epsilonGreedyPolicy(this.epsilon, this.stateMO.getI());
                else
                    idAction =
                            lfa.epsilonGreedyPolicy(this.epsilon, this.stateMO.getI());
                ...
                this.reward += currReward;

                if (isTabular) {
                    // how to access older state from StateManager?
                    qLearning.update(lastState, this.stateMO.getI(),
                            idAction, currReward);
                }
                else {
                    lfa.update(lastState, this.stateMO.getI(),
                            idAction, currReward);
                }
